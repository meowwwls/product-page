(() => {
  ...
  var promos = (function promoHandler(promo) {
    ...
    var promos = { ship4free: 7.99, '15offall': 0.15, '20offmax': 0.2 };
    ...
    return { getDiscount: getDiscount, removePromo: removePromo };
  })();
  ...
  var add = (function addHandler(item) {
    ...
    function createCartItem(item) {
      ...
      html +=
        '<div class="product-page-cart-item"><span class="product-page-cart-itemname">';
      html += '<a href="#"><strong>' + product.name + '</strong></a></span>';
      html +=
        '<span class="product-page-cart-itemdesc">' +
        product.desc.join('</span></span>') +
        '</span>';
      html +=
        '</div></td><td>' +
        priceToString(product.price) +
        '</td><td><button aria-label="decrease quantity" class="qty-decrease btn" title="decrease quantity" data-qty="dec">-</button>';
      html +=
        '<input aria-label="item quantity" type="text" name="item-qty" class="product-page-item-qty" id="" size="2" value="1">';
      html +=
        '<button aria-label="increase quantity" class="qty-increase btn" title="increase quantity" data-qty="inc">+</button></td><td data-cart-item="total">';
      html += priceToString(product.qty * product.price);
      html +=
        '</td><td><button aria-label="remove item from cart" class="product-page-remove btn" title="remove from cart" data-qty="remove"></button></td>';
  ...
  var update = (function handleUpdates() {
    function qty(item, dir) {
      ...
      var id = item.getAttribute('data-product-id');
      var cartIndx = getIndx(id, cart.items);
      var price = cart.items[cartIndx].sale
        ? cart.items[cartIndx].sale
        : cart.items[cartIndx].price;
    ...
    function totalDOM() {
      ...
      els.tax.textContent = priceToString(
        cart.tax * (cart.subtotal - cart.discounts)
      );
      els.shipping.textContent =
        cart.promo === 'ship4free' ? '$0' : priceToString(cart.shipping);
      els.total.textContent = priceToString(cart.total);
    }

    return { qty: qty, total: totalData };
  })();
  ...
  var remove = (function removeHandler() {
    function removeRow(item) {
      var id = item.getAttribute('data-product-id');
      var cartIndx = getIndx(id, cart.items);
      var productBtn = els.productContainer.querySelector(
        '[data-product-id="' + id + '"] .btn'
      );
      var cartItem = els.cartContainer.querySelector(
        '[data-product-id="' + id + '"]'
      );

      cart.items.splice(cartIndx, 1);
      cartItem.classList.add('removing-item');
  ...
  els.cartContainer.addEventListener('click', function(e) {
    ...
    if (e.target.nodeName === 'BUTTON') {
      parent = findParent(e.target, '[data-product-id]');
      dataFunc = e.target.hasAttribute('data-qty')
        ? e.target.getAttribute('data-qty')
        : null;
